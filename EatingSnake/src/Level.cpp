...
bool Level::OnUpdate(Poto::Timestep ts)
{
    ...
    m_LevelTimer += ts;
    if (FACTOR * static_cast<uint>(m_LevelTimer) > m_Snake->GetAge())
    {
        TicToc();
    }
...
void Level::OnRender()
{
    const auto width = m_LevelSize.first + 2, height = m_LevelSize.second + 2;
    ...
    for (float x = farX, xi = 0; xi < width; ++xi, x += m_BlockSide)
    {
        for (float y = farY, yi = 0; yi < height; ++yi, y += m_BlockSide)
        {
    ...
    Poto::Renderer2D::DrawQuad({0, 0}, {width - 2 - 2 * m_BlockPadding, height - 2 - 2 * m_BlockPadding}, white * 0.3f);
